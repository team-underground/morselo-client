					...
					>
						<template v-slot="{ result: { error, data, loading }, query, isLoading }">
							...
							<template v-else-if="data" class="result apollo">
								<card class="mb-1">
									<heading size="heading2" class="mb-3 text-truncate">
										{{
										data.bit.title
										}}
									</heading>
									<div class="md:flex mb-6 md:items-center">
										...
										<div class="w-50">
											...
											<loading-button variant-type="outline" class="items-center" size="small">
												<icon name="bookmark" class="w-6 h-6 text-gray-400 flex-no-shrink"></icon>
