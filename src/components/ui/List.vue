<template>
	<component :is="theme">
		<template v-if="lists.length > 0">
			<li v-for="(list, listIdx) in lists" :key="listIdx" :class="[classes, listCss]" class="flex">
				<div v-if="theme === 'ul'" class="w-8">
					<svg
						class="fill-current w-6 h-6"
						:class="unorderedListColor"
...
export default {
	...
	computed: {
		classes() {
			return [{ "list-none list-style-ul": this.theme === "ul" }];
		}
	}
};
</script>